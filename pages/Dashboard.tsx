...
const Dashboard: React.FC = () => {
  ...
  const relevantHealthRecords = isParent
    ? healthRecords.filter(h => linkedStudentIds.includes(h.studentId))
    : healthRecords;

  const relevantBehaviorRecords = isParent
    ? behaviorRecords.filter(b => linkedStudentIds.includes(b.studentId))
    : behaviorRecords;
  ...
  const behaviorStats = useMemo(() => {
    const positive = relevantBehaviorRecords.filter(b => b.type === 'positive').length;
    const negative = relevantBehaviorRecords.filter(b => b.type === 'negative').length;
  ...
  const alerts = useMemo(() => {
    ...
    // Health Alerts
    relevantHealthRecords.forEach(h => {
    ...
    relevantBehaviorRecords.forEach(b => {
      if (b.type === 'negative') {
  ...
  const getWhatsAppLink = (phone: string, alert: any) => {
    if (!phone) return '#';
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {relevantStudents.map(student => {
              // Academics
              const studentAcademics = academicRecords.filter(r => r.studentId === student.id);
              const latestAcademic = studentAcademics.sort((a, b) => b.schoolYear.localeCompare(a.schoolYear) || b.semester.localeCompare(a.semester))[0];

              // Health
              const studentHealth = healthRecords.filter(r => r.studentId === student.id);
              const latestHealth = studentHealth.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];

              // Behavior
              const studentBehavior = behaviorRecords.filter(r => r.studentId === student.id);
              const positiveCount = studentBehavior.filter(b => b.type === 'positive').length;
              const negativeCount = studentBehavior.filter(b => b.type === 'negative').length;
              const latestBehavior = studentBehavior.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];

              // Attendance
              const today = new Date().toISOString().split('T')[0];
              const todayAttendance = attendanceRecords.find(a => a.studentId === student.id && a.date === today);
                  ...
                  <div className="p-4 bg-gray-50 border-b border-gray-100 flex items-center gap-4">
                    ...
                    <div className="mr-auto flex-shrink-0">
                       <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                          !todayAttendance ? 'bg-gray-100 text-gray-500' :
            ...
            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-3">
              {filteredAlerts.length === 0 ? (
                ...
                filteredAlerts.map(alert => {
                  const studentInfo = students.find(s => s.id === alert.studentId);
                  const guardianPhone = studentInfo?.guardianPhone;

                  return (
                    <div key={alert.id} className={`p-4 rounded-xl border-l-4 shadow-sm hover:shadow-md transition-all bg-white border border-gray-100 group ${
                      alert.type === 'health' 
                        ? 'border-l-red-500' 
                        : 'border-l-orange-500'
                    }`}>
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className={`text-[10px] px-2 py-0.5 rounded-full text-white font-bold shadow-sm ${
                          alert.type === 'health' ? 'bg-red-500' : 'bg-orange-500'
                        }`}>
