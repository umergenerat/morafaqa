...
const Maintenance: React.FC = () => {
  ...
  const canDelete = [UserRole.ADMIN, UserRole.BURSAR].includes(currentUser.role);
  
  // Allow Admin, Bursar, and Supervisor to edit details
  const canEdit = [UserRole.ADMIN, UserRole.BURSAR, UserRole.SUPERVISOR].includes(currentUser.role);
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      const originalRequest = maintenanceRequests.find(r => r.id === editingId);
      if (originalRequest) {
        updateMaintenanceRequest({
          ...
          dateReported: newRequest.dateReported || originalRequest.dateReported
        });
      }
    } else {
      ...
      <div className="flex gap-2 pb-2 overflow-x-auto">
        {(['all', 'pending', 'in_progress', 'completed'] as const).map(status => (
            <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all border whitespace-nowrap ${
                    filterStatus === status 
                    ...
                    <div className="bg-gray-50 p-3 border-t border-gray-100 flex justify-between items-center">
                         {canUpdateStatus ? (
                             ...
                             <div className={`px-2 py-1 rounded-md text-xs font-bold flex items-center gap-1 ${
                                 req.status === 'pending' ? 'text-red-600 bg-red-100' :
                         ...
                         <div className="flex gap-2">
                            {canEdit && (
                                <button 
                                    ...
                                    className="p-1.5 bg-white border hover:bg-blue-50 text-blue-500 rounded-lg shadow-sm transition-colors"
                                    title="تعديل"
                                >
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-gray-200 flex flex-col max-h-[90vh]">
                <div className="p-6 border-b flex justify-between items-center bg-amber-50 rounded-t-2xl">
                    <h3 className="text-xl font-bold text-amber-900 flex items-center gap-2">
                        {editingId ? <Edit2 className="w-6 h-6 text-blue-600" /> : <Plus className="w-6 h-6 text-amber-600" />}
                        {editingId ? 'تعديل الطلب' : t('add_request')}
                    </h3>
                ...
                <div className="p-6 border-t flex gap-3 flex-shrink-0 bg-gray-50 rounded-b-2xl">
                    ...
                    <button onClick={handleSave} className="flex-1 bg-amber-600 text-white font-bold py-3 rounded-xl hover:bg-amber-700 shadow-md transition-colors">
                        {editingId ? t('save') : t('add_request')}
                    </button>
                </div>
            </div>
        </div>
