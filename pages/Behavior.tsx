...
const Behavior: React.FC = () => {
   ...
   const handleSave = () => {
      ...
      const record: BehaviorRecord = {
         ...
         reporter: formData.reporter || currentUser?.name || 'مجهول'
      };
         ...
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex gap-2">
               <button
                  ...
                  className={`px-4 py-2 rounded-xl font-bold text-sm transition-all border flex items-center gap-2 ${showCharts ? 'bg-indigo-50 text-indigo-600 border-indigo-200' : 'bg-white text-gray-600 border-gray-200'
                     }`}
               >
                              ...
                              {filteredStudentsForModal.length === 0 ? (
                                 ...
                                 filteredStudentsForModal.map(s => (
                                    <div
                                       ...
                                       className={`flex items-center gap-3 p-2 cursor-pointer border-b last:border-0 hover:bg-white transition-colors ${formData.studentId === s.id ? 'bg-amber-100 border-amber-200' : ''
                                          }`}
                                    >
