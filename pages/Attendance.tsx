...
import { Save, Upload, Check, Clock, X, LogOut, Calendar, Plus, MapPin, Search, Users, CheckSquare, Square, Filter, FileDown, ArrowLeftRight, History, AlertCircle, ArrowUpDown, ArrowUp, ArrowDown, Home, Edit2, Phone, MessageSquare } from 'lucide-react';
                            ...
                            {!isParent ? (
                              <div className="flex flex-col gap-2">
                                ...
                                {(currentStatus === 'absent' || currentStatus === 'late') && student.guardianPhone && (() => {
                                  const phone = student.guardianPhone.replace(/\s+/g, '').replace(/^0/, '212');
                                  const statusLabel = currentStatus === 'absent' ? 'غائباً' : 'متأخراً';
                                  const msg = encodeURIComponent(`السلام عليكم، نُعلمكم بأن ابنكم/ابنتكم ${student.fullName} (${student.grade}) كان ${statusLabel} اليوم ${today}. يرجى التواصل مع إدارة المؤسسة لمزيد من التوضيح. شكراً.`);
                                  return (
                                    <div className="flex items-center gap-1.5">
                                      <span className="text-xs text-gray-400 font-medium ml-1">تواصل:</span>
                                      <a
                                        href={`https://wa.me/${phone}?text=${msg}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        title="مراسلة عبر واتساب"
                                        className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-bold bg-green-50 text-green-700 border border-green-200 hover:bg-green-100 transition-colors"
                                      >
                                      ...
                                      <a
                                        href={`sms:${student.guardianPhone}?body=${msg}`}
                                        title="إرسال رسالة قصيرة"
                                        className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 transition-colors"
                                      >
                                      ...
                                      <a
                                        ...
                                        title="اتصال مباشر"
                                        className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-bold bg-gray-50 text-gray-700 border border-gray-200 hover:bg-gray-100 transition-colors"
                                      >
